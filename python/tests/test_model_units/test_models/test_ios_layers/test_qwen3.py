...
"""Tests for iOS Qwen3 model parity with HuggingFace."""
...
def _make_ne_qwen3_config(
    ...
    """Create a Qwen3Config for the iOS model.

    Sets rope_scaling=None as required by our iOS model assertion.
...
class TestNEQwen3ForCausalLM:
    """Test iOS Qwen3ForCausalLM against HuggingFace reference."""

    def test_forward_parity_multi_token(self):
        """Multi-token prefill: iOS model matches HF logits."""
        seq_len = 4
        max_seq = 32
        hf_config = _make_hf_qwen3_config(max_position_embeddings=max_seq)
        ne_config = _make_ne_qwen3_config(max_position_embeddings=max_seq)

        hf_model = HFQwen3ForCausalLM(hf_config).to(torch.float32).eval()
        sd = dict(hf_model.state_dict())

        ne_model = Qwen3ForCausalLMForiOS(
    ...
    def test_forward_parity_two_layers(self):
        ...
        seq_len = 4
        max_seq = 32
        hf_config = _make_hf_qwen3_config(num_hidden_layers=2, max_position_embeddings=max_seq)
        ne_config = _make_ne_qwen3_config(num_hidden_layers=2, max_position_embeddings=max_seq)
    ...
    def test_mutate_state_dict_adds_conv2d_dims(self):
        ...
        ne_config = _make_ne_qwen3_config()
        ne_model = Qwen3ForCausalLMForiOS(
            ne_config, model_device="cpu", disable_embedding_quantization=True
        )
...
@pytest.fixture
def base_qwen3_config():
    """Base Qwen3Config with common test parameters."""
    config = Qwen3Config(
        rms_norm_eps=9.87,
        rope_theta=1e5,
        hidden_size=4,
...
class Qwen3iOSComponentTestBase:
    """Base class with common utilities for testing iOS Qwen3 components."""
    ...
    def setup_attention_weights(our_attention, hf_attention, config):
        ...
        Note: Qwen3 uses no bias in attention projections."""
        ...
        # Setup q_norm and k_norm weights (specific to Qwen3)
        q_norm_weight = torch.randn(head_dim)
        k_norm_weight = torch.randn(head_dim)
        our_attention.q_norm.weight = torch.nn.Parameter(q_norm_weight.clone())
        our_attention.k_norm.weight = torch.nn.Parameter(k_norm_weight.clone())
        hf_attention.q_norm.weight = torch.nn.Parameter(q_norm_weight.clone())
        hf_attention.k_norm.weight = torch.nn.Parameter(k_norm_weight.clone())
...
class TestQwen3iOSAttention(Qwen3iOSComponentTestBase):
    def get_model_asset(
        self,
        base_qwen3_config,
        heads: tuple[int, int],
        ...
        hf_rotary = Qwen3RotaryEmbedding(config)
    ...
    def test_hf(
        ...
        model, inputs, expected_output = self.get_model_asset(base_qwen3_config, heads, layer_idx)
    ...
    def test_coreai(
        ...
        model, inputs, _ = self.get_model_asset(base_qwen3_config, heads, layer_idx)
        run_compare_coreai(
            model=model,
            inputs=inputs,
        )
...
class TestQwen3iOSTransformerBlock(Qwen3iOSComponentTestBase):
    def get_model_asset(
        self,
        base_qwen3_config,
        heads: tuple[int, int],
        ...
        hf_block = Qwen3DecoderLayer(config=config, layer_idx=layer_idx)
    ...
    def test_hf(
        ...
        model, inputs, expected_output = self.get_model_asset(
            base_qwen3_config, heads, layer_idx, precision
        )
...
@pytest.mark.slow
class TestQwen3iOSForCausalLM(ForCausalLMTestBase):
    _toy_model_id = "yujiepan/qwen3-tiny-random"
    _model_class = Qwen3ForCausalLMForiOS
